...
def prepare_model():
    ...
    if hf_url is not None:
        model_name = SUPPORTED_HF_MODELS[hf_url]["model_name"]
        model_dir = os.path.join(model_dir, model_name)
        safetensors_file = os.path.join(model_dir, "model.safetensors")
        if os.path.exists(safetensors_file):
            logging.info(f"Model file {safetensors_file} already exists, skipping download.")
        else:
    ...
    if os.path.exists(gguf_path) and os.path.getsize(gguf_path) > 0:
        logging.info(f"Quantized GGUF model already exists at {gguf_path}, skipping quantization.")
    else:
        logging.info(f"Quantized GGUF model not found at {gguf_path}, starting conversion and quantization.")
        ...
        if quant_type.startswith("tl"):
            ...
            # Check if i2s_model already exists to skip quantization
            if os.path.exists(i2s_model) and os.path.getsize(i2s_model) > 0:
                logging.info(f"Quantized i2_s model already exists at {i2s_model}, skipping quantization.")
            else:
                # quantize to i2s
                if platform.system() != "Windows":
                    quantize_cmd = ["./build/bin/llama-quantize", "--token-embedding-type", "f16", f32_model, i2s_model, "I2_S", "1", "1"] if quant_embd else ["./build/bin/llama-quantize", f32_model, i2s_model, "I2_S", "1"]
                else:
...
def parse_args():
    ...
    parser.add_argument("--quant-type", "-q", type=str, help="Quantization type", choices=SUPPORTED_QUANT_TYPES.get(arch, []), default="i2_s")
